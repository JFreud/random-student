...
#STUDENTS = []
#STUDENTS.append(CLASSES[key] for key in CLASSES.keys())
#print STUDENTS

def pick_random(dict):
    period = random.choice(CLASSES.keys())
    return random.choice(CLASSES[period])

def pick_student(period):
    '''Picks random student from the inputted period'''
    studs = CLASSES[period]
    return random.choice(studs)


print pick_random(CLASSES)
print pick_student(8)
